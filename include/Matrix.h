 ...
 ***************************************************************************
 *
 * Matrix class derived from
 *http://codereview.stackexchange.com/questions/43548/fixed-size-matrix-implementation
 *
 */
...
template <typename Ty, int N, int M = N> struct Matrix {
    ...
    Matrix<Ty, N, N> Identity()
    {
        ...
        for (int i = 0; i < N * N; ++i)
            result.flatten[i] = Ty(0);
        for (int i = 0; i < N; ++i)
            result.element[i][i] = Ty(1);
        return result;
    }
};
...
namespace detail {
template <typename Ty, int N, int M> struct inverse;

// Matrix inversion for 2x2 matrix
template <typename Ty> struct inverse<Ty, 2, 2> {
    Matrix<Ty, 2, 2> operator()(const Matrix<Ty, 2, 2>& a)
    {
        Matrix<Ty, 2, 2> result;
        Ty det = a.element[0][0] * a.element[1][1]
...
Matrix<Ty, N, M> operator-(const Matrix<Ty, N, M>& a)
{
    ...
    for (int e = 0; e < N * M; ++e)
        result.flatten[e] = -a.flatten[e];
    return result;
}

#define MATRIX_WITH_MATRIX_OPERATOR(op_symbol, op)                             \
    template <typename Ty, int N, int M>                                       \
    Matrix<Ty, N, M> operator op_symbol(                                       \
        const Matrix<Ty, N, M>& a, const Matrix<Ty, N, M>& b)                  \
    {                                                                          \
        Matrix<Ty, N, M> result;                                               \
        for (int e = 0; e < N * M; ++e)                                        \
